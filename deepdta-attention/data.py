...
# Attention defaults (new vs original DeepDTA). Overridable via CLI.
# attn_heads must divide conv_dim = num_filters * 3 (default 32*3=96 → 4 heads).
ATTN_DEFAULTS = {
    "attn_heads": 4,
    "attn_dropout": 0.1,
}
...
DATASETS = {
    ...
    # Tiny synthetic set for architecture smoke tests (see dummy_data/ and make_dummy_data.py).
    "dummy": {
        "path": str(PKG_DIR / "dummy_data"),
        "is_log": 0,
        "max_smi_len": 40,
        "max_seq_len": 60,
        "smi_kernel": 4,
