"""Train / evaluate / predict DeepDTA with PyTorch."""
...
from deepdta.metrics import concordance_index, evaluate_all, mse
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DeepDTA (PyTorch CNN–CNN)")
    ...
    pred_p.add_argument("--dataset", type=str, default="kiba", choices=["kiba", "davis"])
...
def _make_model(args: argparse.Namespace, spec: dict) -> DeepDTA:
...
def _run_dirs(args: argparse.Namespace) -> tuple[Path, Path]:
    ...
    return REPO_ROOT / "logs" / "deepdta" / stamp, REPO_ROOT / "checkpoints" / "deepdta" / stamp
...
def cmd_evaluate(args: argparse.Namespace) -> None:
    ...
    metrics = evaluate_all(y_true, y_pred, spec["aupr_threshold"])
    print(metrics)
    out = REPO_ROOT / "logs" / "deepdta" / "eval"
