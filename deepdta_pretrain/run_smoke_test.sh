...
echo "=== Unit: attention_pool + DeepDTAPretrain forward ==="
python3 - <<'PY'
import torch
from deepdta_pretrain.embeddings import attention_pool, mean_pool
...
attn = attention_pool(hidden, mask, special)
mean = mean_pool(hidden, mask, special)
assert attn.shape == (4, 32)
assert not torch.allclose(attn, mean), "attention pool should differ from mean pool"
...
out = model(drug, prot)
assert out.shape == (8,)
print("OK: shapes and attention_pool sanity check passed")
PY
...
mkdir -p "$OUT"
echo "=== Train smoke: $DATASET fold 0, 2 epochs, protein-pool=attention ==="
python3 -m deepdta_pretrain train \
  --dataset "$DATASET" \
  --fold 0 \
  --epochs 2 \
  --patience 3 \
  --protein-pool attention \
